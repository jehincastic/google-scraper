...
func getScrapeClient(proxy interface{}) (*http.Client, error) {
	switch val := proxy.(type) {
	case string:
		proxyURL, e := url.Parse(val)
		if e != nil {
			return nil, e
		}
		client := &http.Client{
...
func scrapeClientRequest(url string, proxy interface{}) (*http.Response, error) {
	baseClient, err := getScrapeClient(proxy)
	if err != nil {
		return nil, err
	}
...
func googleResultParsing(resp *http.Response, count int) ([]SearchResult, error) {
...
func buildGoogleUrls(
	...
	toScrape := []string{}
	searchTerm := strings.Trim(query, " ")
	searchTerm = strings.Replace(searchTerm, " ", "+", -1)
	domain, ok := googleDomains[countryCode]
	if !ok {
	...
	for i := 0; i < pages; i++ {
		start := i * count
		url := fmt.Sprintf(
			"%s%s&num=%d&start=%d&hl=%s&filter=0",
			domain,
			searchTerm,
			count,
			start,
...
func GoogleScrape(
	...
	resultCounter := 0
	googlePages, err := buildGoogleUrls(query, countryCode, langCode, pages, count)
	if err != nil {
		return nil, err
	}
	for _, page := range googlePages {
		...
		data, err := googleResultParsing(resp, resultCounter)
		if err != nil {
			return nil, err
		}
...
func main() {
	res, err := GoogleScrape("Castic Jehin", "com", "en", 1, 30, 10, nil)
	if err != nil {
		fmt.Println(err)
	} else {
